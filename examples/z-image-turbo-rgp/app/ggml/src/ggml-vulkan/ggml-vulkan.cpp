...
static std::string ggml_vk_debug_label_name(const ggml_tensor * node, const char * fusion_name) {
    std::string label;
    if (fusion_name && fusion_name[0] != '\0') {
        label += fusion_name;
        label += " ";
    }
    label += ggml_op_name(node->op);
    if (node->name[0] != '\0') {
        label += " ";
        label += node->name;
    }
...
static uint32_t ggml_vk_guess_split_k(ggml_backend_vk_context * ctx, uint32_t m, uint32_t n, uint32_t k, bool disable_split_k, const vk_pipeline& pipeline) {
    ...
    const char * ggml_vk_force_split_k = getenv("GGML_VK_FORCE_SPLIT_K");
    if (ggml_vk_force_split_k != nullptr && *ggml_vk_force_split_k != '\0') {
        const int forced = std::atoi(ggml_vk_force_split_k);
        if (forced > 0) {
...
static bool ggml_vk_is_k_quant_type(ggml_type type) {
    switch (type) {
        case GGML_TYPE_Q2_K:
        case GGML_TYPE_Q3_K:
        case GGML_TYPE_Q4_K:
...
static bool ggml_vk_prefer_extra_split_k_for_amd_k_quants(ggml_backend_vk_context * ctx, ggml_type src0_type, uint32_t n, uint32_t k) {
    if (ctx->device->vendor_id != VK_VENDOR_ID_AMD) {
        return false;
    }
    if (!ggml_vk_is_k_quant_type(src0_type)) {
        return false;
    }
    // Real diffusion profiling on RADV NAVI31 shows low occupancy and WAIT-heavy hotspots
    // on large-K qK matmuls. A modest split_k improves parallelism for these shapes without
    // needing the more invasive profiling-only execution changes.
    return k >= 8192 && n >= 128 && n <= 512;
}

static int ggml_vk_forced_k_quant_shader_size() {
    const char * value = getenv("GGML_VK_FORCE_K_QUANT_SHADER_SIZE");
    if (value == nullptr || *value == '\0') {
        return -1;
    }
    int forced = std::atoi(value);
    return forced >= 0 && forced <= 2 ? forced : -1;
}

static vk_pipeline ggml_vk_select_forced_matmul_pipeline(vk_matmul_pipeline& mmp, bool aligned, int shader_size) {
    switch (shader_size) {
        case 0:
...
static vk_pipeline ggml_vk_guess_matmul_pipeline(ggml_backend_vk_context * ctx, vk_matmul_pipeline& mmp, uint32_t m, uint32_t n, bool aligned, ggml_type src0_type, ggml_type src1_type) {
    ...
    const int forced_k_quant_shader_size = ggml_vk_forced_k_quant_shader_size();
    if (forced_k_quant_shader_size >= 0 && ggml_vk_is_k_quant_type(src0_type)) {
        vk_pipeline forced = ggml_vk_select_forced_matmul_pipeline(mmp, aligned, forced_k_quant_shader_size);
        if (forced != nullptr) {
            return forced;
        }
    }
...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool disable_split_k) {
    ...
    if (split_k == 1 &&
        getenv("GGML_VK_FORCE_SPLIT_K") == nullptr &&
        ggml_vk_prefer_extra_split_k_for_amd_k_quants(ctx, src0->type, ne11, ne10)) {
        split_k = 3;
    }
...
static ggml_status ggml_backend_vk_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    if (vk_perf_logger_enabled) {
        ...
        if (vk_instance.debug_utils_support) {
            vk::DebugUtilsLabelEXT dul = {};
            dul.pLabelName = "ggml_backend_vk_graph_compute/cmd";
            dul.color = std::array<float,4>{0.9f, 0.9f, 0.9f, 1.0f};
            vk_instance.pfn_vkCmdBeginDebugUtilsLabelEXT(
                static_cast<VkCommandBuffer>(compute_ctx->s->buffer->buf),
    ...
    const char * ggml_vk_profile_nodes_per_submit = getenv("GGML_VK_PROFILE_NODES_PER_SUBMIT");
    if (ggml_vk_profile_nodes_per_submit != nullptr) {
        nodes_per_submit = std::max(1, atoi(ggml_vk_profile_nodes_per_submit));
    }
    const char * ggml_vk_profile_matmul_bytes_per_submit = getenv("GGML_VK_PROFILE_MATMUL_BYTES_PER_SUBMIT");
    if (ggml_vk_profile_matmul_bytes_per_submit != nullptr) {
        mul_mat_bytes_per_submit = std::stoull(ggml_vk_profile_matmul_bytes_per_submit);
    }
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (vk_instance.debug_utils_support) {
            compute_ctx = ggml_vk_get_compute_ctx(ctx);
            if (!label_group_open) {
                const std::string group_label_name =
                    "ggml_backend_vk_graph_compute/group submit=" + std::to_string(submit_count) +
                    " node=" + std::to_string(i);
                vk::DebugUtilsLabelEXT group_dul = {};
                group_dul.pLabelName = group_label_name.c_str();
                group_dul.color = std::array<float,4>{0.8f, 0.5f, 0.1f, 1.0f};
                vk_instance.pfn_vkCmdBeginDebugUtilsLabelEXT(
                    static_cast<VkCommandBuffer>(compute_ctx->s->buffer->buf),
                    reinterpret_cast<VkDebugUtilsLabelEXT*>(&group_dul));
                label_group_open = true;
            }
            const std::string label_name = ggml_vk_debug_label_name(cgraph->nodes[i], fusion_string);
            vk::DebugUtilsLabelEXT dul = {};
            dul.pLabelName = label_name.c_str();
            dul.color = std::array<float,4>{0.2f, 0.7f, 1.0f, 1.0f};
            vk_instance.pfn_vkCmdInsertDebugUtilsLabelEXT(
                static_cast<VkCommandBuffer>(compute_ctx->s->buffer->buf),
                reinterpret_cast<VkDebugUtilsLabelEXT*>(&dul));
        }
        ...
        if (vk_instance.debug_utils_support && label_group_open && enqueued &&
            (label_group_nodes >= 16 || submit || i + ctx->num_additional_fused_ops >= last_node)) {
            vk_instance.pfn_vkCmdEndDebugUtilsLabelEXT(
                static_cast<VkCommandBuffer>(compute_ctx->s->buffer->buf));
            label_group_open = false;
            label_group_nodes = 0;
        }
    ...
    if (vk_instance.debug_utils_support && label_group_open) {
        compute_ctx = ggml_vk_get_compute_ctx(ctx);
        vk_instance.pfn_vkCmdEndDebugUtilsLabelEXT(
            static_cast<VkCommandBuffer>(compute_ctx->s->buffer->buf));
    }
