...
uint32_t lds_bank_value(const std::vector<uint32_t>& input, uint32_t idx, uint32_t multiplier, uint32_t bias) {
...
int main(int argc, char **argv) {
    const char *shader_path = (argc >= 2) ? argv[1] : "./build/shaders/lds_clean.comp.spv";
    const char *variant = (argc >= 3) ? argv[2] : "lds_clean";
    ...
    app_info.pApplicationName = "vulkan-to-7900xtx-isa-ch06";
    ...
    uint32_t checksum = 0;
    {
        ...
        for (uint32_t idx = 0; idx < kElementCount; ++idx) {
            uint32_t expected = lds_bank_value(input_values, idx, kMultiplier, kBias);
