...
uint32_t baseline_value(uint32_t idx, uint32_t multiplier, uint32_t bias) {
    return idx * multiplier + bias;
}

uint32_t reg_pressure_value(uint32_t idx, uint32_t multiplier, uint32_t bias) {
    uint32_t a0 = idx;
    uint32_t a1 = idx + bias;
    uint32_t a2 = idx ^ multiplier;
    uint32_t a3 = idx * 3u + 1u;

    uint32_t b0 = a0 * 5u + bias;
    uint32_t b1 = a1 * 7u + multiplier;
    uint32_t b2 = a2 * 9u + (bias ^ multiplier);
    uint32_t b3 = a3 * 11u + idx;

    uint32_t c0 = b0 ^ (b1 >> 1u);
    uint32_t c1 = b1 ^ (b2 >> 2u);
    uint32_t c2 = b2 ^ (b3 >> 3u);
    uint32_t c3 = b3 ^ (b0 >> 1u);
    ...
    uint32_t adjust = (d0 ^ d1 ^ d2 ^ d3) & 31u;
    return idx * multiplier + bias + adjust;
}
...
int main(int argc, char **argv) {
    ...
    app_info.pApplicationName = "vulkan-to-7900xtx-isa-ch02";
    ...
    uint32_t checksum = 0;
    {
        ...
        for (uint32_t idx = 0; idx < kElementCount; ++idx) {
            uint32_t expected = 0;
            if (std::string(variant) == "baseline") {
                expected = baseline_value(idx, kMultiplier, kBias);
            } else if (std::string(variant) == "reg_pressure") {
                expected = reg_pressure_value(idx, kMultiplier, kBias);
            } else {
                vkUnmapMemory(device, buffer.memory);
                fail(std::string("unknown variant: ") + variant);
            }
