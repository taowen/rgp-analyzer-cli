...
sleep 1
env MESA_VK_TRACE=rgp MESA_VK_TRACE_PER_SUBMIT=1 RADV_THREAD_TRACE_BUFFER_SIZE=67108864 "${binary}" "${shader}" cooperative "${repeats}"
find /tmp -maxdepth 1 -type f -name '*.rgp' -printf '%f\n' | sort > "${after_list}"
mapfile -t new_files < <(comm -13 "${before_list}" "${after_list}")
if [[ "${#new_files[@]}" -eq 0 ]]; then echo "no new .rgp files found in /tmp" >&2; exit 1; fi
latest_path=""
latest_size=-1
for name in "${new_files[@]}"; do path="/tmp/${name}"; [[ -f "${path}" ]] || continue; size="$(stat -c '%s' "${path}")"; if (( size > latest_size )); then latest_size="${size}"; latest_path="${path}"; fi; done
[[ -n "${latest_path}" ]] || { echo "failed to choose a capture file" >&2; exit 1; }
cp "${latest_path}" "${capture_dir}/cooperative.rgp"
echo "captured ${capture_dir}/cooperative.rgp size_bytes=${latest_size}"
