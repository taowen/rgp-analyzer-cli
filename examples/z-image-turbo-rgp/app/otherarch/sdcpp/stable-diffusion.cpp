...
static void zimage_write_phase_marker(const char * phase_name) {
    const char * marker_dir = std::getenv("ZIMAGE_PHASE_MARKER_DIR");
    if (marker_dir == nullptr || marker_dir[0] == '\0' || phase_name == nullptr || phase_name[0] == '\0') {
        return;
    }
    ...
    const auto epoch_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(now.time_since_epoch()).count();
    std::ofstream out(std::filesystem::path(marker_dir) / (std::string(phase_name) + ".txt"));
    if (!out) {
        return;
    }
    out << epoch_ns << "\n";
...
enum class zimage_stop_after_phase_t {
    none,
    condition,
    diffusion,
};

static zimage_stop_after_phase_t zimage_get_stop_after_phase() {
    const char * value = std::getenv("ZIMAGE_STOP_AFTER_PHASE");
    if (value == nullptr || value[0] == '\0') {
        return zimage_stop_after_phase_t::none;
    }

    const std::string phase(value);
    if (phase == "condition") {
        return zimage_stop_after_phase_t::condition;
    }
    if (phase == "diffusion") {
        return zimage_stop_after_phase_t::diffusion;
    }
    return zimage_stop_after_phase_t::none;
}

static sd_image_t * zimage_make_phase_only_results(struct ggml_context * work_ctx, int batch_count) {
...
sd_image_t* generate_image_internal(sd_ctx_t* sd_ctx,
    ...
    zimage_write_phase_marker("phase_condition_begin");
    ...
    int64_t t1 = ggml_time_ms();
    zimage_write_phase_marker("phase_condition_end");
    ...
    if (zimage_get_stop_after_phase() == zimage_stop_after_phase_t::condition) {
        LOG_INFO("ZIMAGE_STOP_AFTER_PHASE=condition, returning after conditioning");
        return zimage_make_phase_only_results(work_ctx, batch_count);
    }
    ...
    for (int b = 0; b < batch_count; b++) {
        zimage_write_phase_marker("phase_diffusion_begin");
        ...
        zimage_write_phase_marker("phase_diffusion_end");
        if (x_0 != nullptr) {
    ...
    if (zimage_get_stop_after_phase() == zimage_stop_after_phase_t::diffusion) {
        LOG_INFO("ZIMAGE_STOP_AFTER_PHASE=diffusion, returning after latent generation");
        return zimage_make_phase_only_results(work_ctx, batch_count);
    }
    ...
    for (size_t i = 0; i < final_latents.size(); i++) {
        zimage_write_phase_marker("phase_vae_begin");
        t1                      = ggml_time_ms();
        ...
        zimage_write_phase_marker("phase_vae_end");
