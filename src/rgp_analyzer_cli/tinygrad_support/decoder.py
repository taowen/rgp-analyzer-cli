...
from ..vendor.tinygrad_sqtt import decode, format_packet
...
def analyze_tinygrad_sqtt(
    ...
    *,
    ...
    return {"tinygrad_vendor": "vendored", "stream_count": len(streams), "streams": streams}


def render_tinygrad_sqtt(result: dict[str, Any], limit: int = 20, preview_limit: int = 20) -> str:
    lines = [f"tinygrad_sqtt: vendor={result.get('tinygrad_vendor', 'vendored')}"]
