...
from .helpers import getenv, colored
from .dsl import BitField, FixedBitField, Inst, bits
from .ins import s_endpgm  # same encoding as RDNA4
...
def map_insts(data:bytes, lib:bytes, target:str) -> Iterator[tuple[PacketType, InstructionInfo|None]]:
  ...
  # map pcs to insts
  raise RuntimeError("map_insts is not supported in vendored tinygrad SQTT mode")
...
if __name__ == "__main__":
  import sys, pickle
  raise RuntimeError("__main__ entry is not supported in vendored tinygrad SQTT mode")
