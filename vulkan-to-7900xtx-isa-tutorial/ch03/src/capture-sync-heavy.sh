...
env MESA_VK_TRACE=rgp MESA_VK_TRACE_PER_SUBMIT=1 RADV_THREAD_TRACE_BUFFER_SIZE=67108864 "${binary}" "${shader}" sync_heavy "${repeats}"
find /tmp -maxdepth 1 -type f -name '*.rgp' -printf '%f\n' | sort > "${after_list}"
...
cp "${latest_path}" "${capture_dir}/sync_heavy.rgp"
echo "captured ${capture_dir}/sync_heavy.rgp size_bytes=${latest_size}"
