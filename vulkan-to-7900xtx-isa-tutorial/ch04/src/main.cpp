...
int main(int argc, char **argv) {
    const char *shader_path = (argc >= 2) ? argv[1] : "./build/shaders/scalar_matmul.comp.spv";
    const char *variant = (argc >= 3) ? argv[2] : "scalar";
    const uint32_t dispatch_repeats = (argc >= 4) ? static_cast<uint32_t>(std::strtoul(argv[3], nullptr, 10)) : 64u;
    const bool use_cooperative = std::strcmp(variant, "cooperative") == 0;

    constexpr uint32_t kMatrixSize = 16;
    ...
    std::vector<float> host_b_coop_f32(kElementCount);
    ...
    for (uint32_t i = 0; i < kElementCount; ++i) {
        host_a_f32[i] = static_cast<float>((i % 7u) + 1u);
        host_b_f32[i] = static_cast<float>(((i * 3u) % 11u) + 1u);
    }
    for (uint32_t row = 0; row < kMatrixSize; ++row) {
        for (uint32_t col = 0; col < kMatrixSize; ++col) {
            host_b_coop_f32[col * kMatrixSize + row] = host_b_f32[row * kMatrixSize + col];
        }
    }
    for (uint32_t i = 0; i < kElementCount; ++i) {
        host_a[i].bits = float_to_half_bits(host_a_f32[i]);
        host_b[i].bits = float_to_half_bits(use_cooperative ? host_b_coop_f32[i] : host_b_f32[i]);
    }
    cpu_matmul(host_a_f32, host_b_f32, host_c_expected, kMatrixSize);

    VkApplicationInfo app_info{};
    app_info.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    app_info.pApplicationName = "vulkan-to-7900xtx-isa-ch04";
    ...
    if (use_cooperative && (!has_cooperative_matrix_extension || !cooperative_matrix_features.cooperativeMatrix || !has_target_shape)) {
        fail("cooperative variant requested, but 16x16x16 f16->f32 subgroup cooperative matrix support is unavailable");
    }
    ...
    for (uint32_t i = 0; i < std::max(1u, dispatch_repeats); ++i) {
        ...
        if (use_cooperative) {
            ...
            vkCmdDispatch(command_buffer, 2, 2, 1);
        }
    }
