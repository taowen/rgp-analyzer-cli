...
int main(int argc, char ** argv) {
    if (argc != 4) {
        fail("usage: ch13_runtime_gguf_attn_qk_f32w_probe <runtime.gguf> <layer_probe_dir> <layer_idx>");
    }
    ...
    ggml_set_name(inp, "attn_qk_f32w.input");
    ggml_set_input(inp);

    ggml_tensor * inp_pos = ggml_new_tensor_1d(compute.ctx, GGML_TYPE_I32, seq_len);
    ggml_set_name(inp_pos, "attn_qk_f32w.inp_pos");
    ggml_set_input(inp_pos);

    ggml_tensor * attn_norm_weight = rt.get_tensor(layer_key(il, "attn_norm.weight").c_str());
    ggml_tensor * q_proj_weight = ggml_cast(compute.ctx, rt.get_tensor(layer_key(il, "q_proj.weight").c_str()), GGML_TYPE_F32);
    ggml_tensor * k_proj_weight = ggml_cast(compute.ctx, rt.get_tensor(layer_key(il, "k_proj.weight").c_str()), GGML_TYPE_F32);
    ...
    ggml_set_name(attn_out, "attn_qk_f32w.attn_out");
    ggml_set_name(o_proj, "attn_qk_f32w.o_proj");
    ggml_set_name(attn_residual, "attn_qk_f32w.attn_residual");
    ...
    auto * inp_t = ggml_graph_get_tensor(gf, "attn_qk_f32w.input");
    auto * pos_t = ggml_graph_get_tensor(gf, "attn_qk_f32w.inp_pos");
    if (!inp_t || !pos_t) fail("missing graph inputs");
    ggml_backend_tensor_set(inp_t, input_data.data(), 0, input_data.size() * sizeof(float));
    ...
    const auto attn_diff = diff_stats(tensor_to_f32(ggml_graph_get_tensor(gf, "attn_qk_f32w.attn_out")), read_f32(probe_dir + "/attn_out_ref_f32.bin"), "attn_out");
    const auto oproj_diff = diff_stats(tensor_to_f32(ggml_graph_get_tensor(gf, "attn_qk_f32w.o_proj")), read_f32(probe_dir + "/o_proj_ref_f32.bin"), "o_proj");
    const auto resid_diff = diff_stats(tensor_to_f32(ggml_graph_get_tensor(gf, "attn_qk_f32w.attn_residual")), read_f32(probe_dir + "/attn_residual_ref_f32.bin"), "attn_residual");

    std::cout << "probe.layer=" << il << "\n";
    std::cout << "probe.mode=qk_weight_f32\n";
    std::cout << "attn_qk_f32w.attn_out.max_abs_diff=" << attn_diff.max_abs_diff << "\n";
    std::cout << "attn_qk_f32w.attn_out.mean_abs_diff=" << attn_diff.mean_abs_diff << "\n";
    std::cout << "attn_qk_f32w.o_proj.max_abs_diff=" << oproj_diff.max_abs_diff << "\n";
    std::cout << "attn_qk_f32w.o_proj.mean_abs_diff=" << oproj_diff.mean_abs_diff << "\n";
    std::cout << "attn_qk_f32w.attn_residual.max_abs_diff=" << resid_diff.max_abs_diff << "\n";
    std::cout << "attn_qk_f32w.attn_residual.mean_abs_diff=" << resid_diff.mean_abs_diff << "\n";
    std::cout << "attn_qk_f32w_probe_ok=true\n";
    ggml_backend_sched_reset(rt.sched);
    return 0;
}
