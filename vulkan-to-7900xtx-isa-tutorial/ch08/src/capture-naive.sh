...
MESA_VK_TRACE=rgp MESA_VK_TRACE_PER_SUBMIT=1 bash "${script_dir}/run-naive.sh" "${1:-64}" >/tmp/ch08-naive.log 2>&1
find /tmp -maxdepth 1 -name '*.rgp' -printf '%f\n' | sort > "${after}" || true
...
cp "${new_file}" "${captures_dir}/attention_naive.rgp"
echo "captured ${new_file} -> ${captures_dir}/attention_naive.rgp"
