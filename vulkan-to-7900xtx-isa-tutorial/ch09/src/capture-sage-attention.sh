...
MESA_VK_TRACE=rgp MESA_VK_TRACE_PER_SUBMIT=1 bash "${script_dir}/run-sage-attention.sh" "${1:-64}" >/tmp/ch09-sage-attention.log 2>&1
find /tmp -maxdepth 1 -name '*.rgp' -printf '%f\n' | sort > "${after}" || true
...
cp "${new_file}" "${captures_dir}/sage_attention.rgp"
echo "captured ${new_file} -> ${captures_dir}/sage_attention.rgp"
